...
// Generated by metaharness — the `federated-mcp-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx federated-mcp-agent`
...
// "bin" + "files" fields in package.json), so `npx federated-mcp-agent` works the moment
...
const HARNESS_NAME = 'federated-mcp-agent';

/** `federated-mcp-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `federated-mcp-agent doctor` — verify the install end-to-end (kernel + host resolve). */
